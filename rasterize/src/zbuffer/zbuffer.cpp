...
float ZBuffer::get_depth(int x, int y) const
{
...
void ZBuffer::rasterize(const Triangle &tri)
{
    ...
    tri_bb.foreach_pixel([&](int x, int y) {
        ...
        if (tri.contains_point(point)) {
            float depth = tri.interpolate_depth(point);
...
void ZBuffer::set_pixel(int x, int y, float depth, const RgbColor &color)
{
...
void ZBuffer::set_pixel_with_bvh(int x, int y, float depth, const RgbColor &color)
{
...
bool ZBuffer::is_in_zbuffer(const BoundingRect &rect, float depth) const
{
    return false;
}

bool ZBuffer::is_in_zbuffer(PixelIndex index, const BoundingRect &rect, float depth) const
{
    return false;
}

bool ZBuffer::is_in_zbuffer(const Triangle &tri, float depth) const
{
    return false;
}
